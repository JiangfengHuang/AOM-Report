...
For the stochastic methods there are several algorithms, one of the most popular being genetic algorithms~\cite{Genetic_tutorial}. These algorithms try to find an optimal solution by simulating an evolution process. A random set of initially polynomials is applied to the mirror and an iterative process selects the best solutions. An implementation of a genetic algorithm in adaptive optics begins with a population of typically random chromosomes.

They start from a population of possible solutions and save some of the strongest elements that are those selected to survive, which means that are able to be reproduced and give the next generations. The probability in which the inferior solutions survive and reproduce is smaller. In general terms, the basic steps for this algorithms are: 1)Chose a random set of possible solutions, 2)Selection function, 3)Reproduce the function in order to create the new generations, 4)Evaluate the possible solutions, 5)Repeat from step 2. For the sake of brevity, we recommend these references for a further reading \cite{Genetic_Algor1},\cite{Genetic_Algor2} and \cite{Genetic_Algor3}. Now we are going to proceed to explain a general description of the image-based techniques. 

 These genetic algorithms do not require any preliminary information about the system but they are more time consuming than model-based approaches.
...
%The focus of this section is therefore a general 
